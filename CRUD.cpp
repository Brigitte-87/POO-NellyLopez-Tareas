...
// Aqui de deben de colocar las funciones para mandarlas a llamar en el main
void leer();
void crear();
main(){
...
void leer(){
	//cls para limpiar
	system("cls");
	// tambien se puede utlizar r+b para saltarse el paso del if
	FILE* archivo = fopen(nombre_archivo,"rb");
	// si queremos que vea si el false por si no exite la carpeta se utiliza !
	if(!archivo){
	...
	do{
		...
		fread(&estudiante,sizeof(Estudiante),1,archivo);
		id +=1;
	}while(feof(archivo)==0); // tambien se puede utilizar while(!feof(arvhico));
	fclose(archivo);
}
void crear (){
